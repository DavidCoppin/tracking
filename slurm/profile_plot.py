...
n_steps = n_tasks = n_nodes = 0
for (step_name, step) in steps.items():
    if step_name == 'batch' and not INCLUDE_BATCH:
        continue
    n_steps += 1
...
for ((series_name, pattern, sign), colour, x, dx, y) in cio_plots:
    cio_subplot.plot(x, y/cio_scale, color=colour, linestyle=pattern, label=series_name)
cio_subplot.legend(loc='best')
